...
function isPalindrome(str) {
  const lcstr = str
    ...
    .join('')
    .toLowerCase();
  for (let i = 0, j = lcstr.length - 1; i <= j; i += 1, j -= 1) {
    if (lcstr.at(i) !== lcstr.at(j)) {
      return false;
    }
  }
  return true;
 ...
 *
 * @example
 *   getStringFromTemplate('JohnDoe') => 'Hello, John Doe!'
 *   getStringFromTemplate('ChuckNorris') => 'Hello, Chuck Norris!'
 */
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (' ?!.;-,'.includes(str[i])) {
      ...
      let j = set.indexOf(str[i].toLowerCase()) + 13;
      if (j >= set.length) {
        j -= set.length;
      }
      res.push(str[i] === str[i].toLowerCase() ? set[j] : set[j].toUpperCase());
    }
  }
  return res.join('');
}
